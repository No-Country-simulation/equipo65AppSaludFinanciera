...
public class ClienteMlService {
    ...
    /**
     * Estado del servicio de inferencia, para `GET /api/v1/salud`.
     *
     * A diferencia del resto de metodos, este **no lanza**: un ML caido es una
     * respuesta valida de un endpoint de salud, no un error de la peticion. Y
     * no reintenta: el healthcheck lo consultan el contenedor y el tunel cada
     * pocos segundos, y un reintento solo duplicaria la espera antes de dar la
     * misma noticia.
     */
    public SaludMl salud() {
        try {
            @SuppressWarnings("unchecked")
            Map<String, Object> cuerpo = http.get()
                    .uri("/interno/v1/salud")
                    .headers(h -> {
            ...
            if (cuerpo == null) return SaludMl.caido("respuesta vacia");
            return new SaludMl(
                    String.valueOf(cuerpo.getOrDefault("estado", "desconocido")),
                    versionDe(cuerpo),
                    null);
        } catch (Exception e) {
            log.warn("El servicio de ML no responde al healthcheck: {}", e.toString());
            return SaludMl.caido(e.getClass().getSimpleName());
        }
    }

    /** La version sale del bloque del clasificador, que es el modelo que siempre esta en uso. */
    @SuppressWarnings("unchecked")
    private static String versionDe(Map<String, Object> cuerpo) {
        Object modelo = cuerpo.get("modelo_transacciones");
        if (modelo instanceof Map<?, ?> m && m.get("version") != null) {
    ...
    /** Lo que `GET /api/v1/salud` reporta del ML. `detalle` solo va si esta caido. */
    public record SaludMl(String estado, String modeloVersion, String detalle) {
        public static SaludMl caido(String detalle) {
            return new SaludMl("caido", null, detalle);
        }
    }
