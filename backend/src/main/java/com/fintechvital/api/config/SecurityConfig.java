...
import com.fintechvital.api.security.JwtFiltro;
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // El clasificador suelto del enunciado es PUBLICO y cuelga de
                // /transacciones. Va declarado ANTES que el resto del recurso
                // porque en Spring Security gana la primera regla que casa, no
                // la mas especifica: al reves, exigiria token y el jurado no
                // podria probarlo con un curl.
                .requestMatchers("/api/v1/transacciones/clasificar").permitAll()
                // Datos del usuario: movimientos, calendario, analisis guardado,
                // metas y presupuestos. Todos filtran por el token (RN9).
                .requestMatchers("/api/v1/transacciones/**", "/api/v1/movimientos/**",
                                 "/api/v1/eventos/**", "/api/v1/analisis",
                                 "/api/v1/analisis/**", "/api/v1/resumen/**",
                                 "/api/v1/metas/**", "/api/v1/presupuestos/**").authenticated()
