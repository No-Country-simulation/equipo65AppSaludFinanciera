...
# Uso:
#   ./scripts/linux/web-docker.sh            # build + up
#   ./scripts/linux/web-docker.sh --rebuild  # fuerza rebuild sin cache
#   ./scripts/linux/web-docker.sh --down     # detiene y elimina el contenedor
...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
  ...
  echo "ERROR: ni Docker ni Podman responden. Arranca o instala uno de los dos." >&2
  exit 1
fi
