...
#   ./scripts/linux/verificar-requisitos.sh --instalar-podman   # instala Podman si falta
...
if [[ -z "$motor" ]]; then
  if ! command -v docker >/dev/null 2>&1 && ! command -v podman >/dev/null 2>&1; then
    falta "Ni Docker ni Podman. Opciones: Docker -> https://docs.docker.com/engine/install/  o instalar Podman desde aqui."
    instalar="$INSTALAR_PODMAN"
    if [[ "$instalar" == "no" && -t 0 ]]; then
      read -r -p "Quieres que instale Podman ahora? (s/N) " r
      [[ "$r" == "s" || "$r" == "S" ]] && instalar=si
    fi
    if [[ "$instalar" == "si" ]]; then
      if instalar_podman; then motor="podman"; FALLAS=$((FALLAS - 1)); fi
    else
      info "Puedes instalarlo despues con: ./scripts/linux/verificar-requisitos.sh --instalar-podman"
    fi
  else
...
if [[ -n "$motor" ]]; then
  # En Podman, `compose` lo provee un binario externo: si falta, el stack no
...
# Java solo hace falta para compilar la API FUERA del contenedor: la imagen trae
# su propio JDK 21.
