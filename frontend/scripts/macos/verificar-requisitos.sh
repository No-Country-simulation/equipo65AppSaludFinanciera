...
if command -v git >/dev/null 2>&1; then
  ...
  falta "Git no esta instalado (xcode-select --install, o brew install git)"
fi
...
if command -v node >/dev/null 2>&1; then
  ...
  if [[ "$major" -ge 20 ]]; then
    ...
    falta "Node.js v$ver es viejo: se necesita 20+ (LTS) -> brew install node@22 o https://nodejs.org"
  fi
else
  falta "Node.js no esta instalado (se necesita 20+ LTS) -> brew install node@22 o https://nodejs.org"
fi
...
if command -v docker >/dev/null 2>&1; then
  if docker info >/dev/null 2>&1; then
    ...
    aviso "Docker instalado pero el daemon NO responde (abre Docker Desktop y espera a que arranque)"
  fi
else
...
if [[ -z "$motor" ]] && command -v podman >/dev/null 2>&1; then
  if podman info >/dev/null 2>&1; then
    ...
    aviso "Podman esta instalado pero no responde: correr 'podman machine start'"
  fi
fi
...
info "Virtualizacion: macOS trae Hypervisor.framework integrado (no hay nada que habilitar)"
