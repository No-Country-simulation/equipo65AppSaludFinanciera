...
export default function PantallaPresupuestos() {
  ...
  const { temaActivo } = useTheme(); // 2. Extraemos el tema activo
    ...
    <View style={{ flex: 1, backgroundColor: temaActivo.canvas }}>
      <Hero paddingTop={insets.top + 14}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
          <Text style={[s.titulo, { color: temaActivo.blanco }]}>{t('presupuestos.titulo')}</Text>
          <Boton texto={t('presupuestos.nuevo')} variante="claro" onPress={() => { setCategoria(''); setLimite(''); setModal(true); }} />
        </View>
        <Text style={[s.subtitulo, { color: temaActivo.blanco }]}>{t('presupuestos.subtitulo')}</Text>
      </Hero>
        ...
        <EstadoCarga cargando={cargando} error={error} recargar={recargar}>
          {datos && datos.presupuestos.length > 0 ? (
            <Tarjeta>
              ...
              <Text style={[s.totalCifra, { color: temaActivo.tinta }]}>
                {formatearMoneda(totalGastado, moneda, idioma)}
                <Text style={{ fontFamily: Fuentes.cuerpo, fontSize: 15, color: temaActivo.apagado }}>
                  {'  '}{t('presupuestos.de')} {formatearMoneda(totalLimite, moneda, idioma)}
                </Text>
              </Text>
              <View style={[s.pistaTotal, { backgroundColor: temaActivo.canvas2 }]}>
                <View
                  style={{
                    width: `${Math.min(100, fraccionTotal * 100)}%`,
                    height: '100%',
                    borderRadius: 999,
                    backgroundColor: fraccionTotal >= 1 ? temaActivo.riesgo : fraccionTotal >= 0.8 ? temaActivo.alertaFondo : temaActivo.menta,
                  }}
                />
              </View>
              <Text style={{ fontFamily: Fuentes.cuerpo, fontSize: 11, color: temaActivo.apagado, textAlign: 'right', marginTop: 4 }}>
          ...
          {datos && datos.presupuestos.length === 0 ? (
            <Text style={[s.vacio, { color: temaActivo.apagado }]}>{t('presupuestos.vacio')}</Text>
          ) : (
            [...(datos?.presupuestos ?? [])]
      ...
      <Modal visible={modal} animationType="slide" transparent>
        <View style={s.fondoModal}>
          <View style={[s.modal, { backgroundColor: temaActivo.canvas }]}>
            <Text style={[s.modalTitulo, { color: temaActivo.tinta }]}>{t('presupuestos.nuevo')}</Text>
            <Text style={[s.etiqueta, { color: temaActivo.tintaSuave }]}>{t('presupuestos.categoria')}</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingVertical: 2 }}>
              {disponibles.map((c) => (
                <Pressable
                  ...
                  style={[
                    s.chipCat,
                    { borderColor: temaActivo.linea, backgroundColor: temaActivo.tarjeta },
...
const s = StyleSheet.create({
  ...
  subtitulo: { fontFamily: Fuentes.cuerpo, fontSize: 12, opacity: 0.65, marginTop: 6 },
  totalCifra: { fontFamily: Fuentes.titulo, fontSize: 22 },
  pistaTotal: { height: 12, borderRadius: 999, overflow: 'hidden', marginTop: 10 },
  vacio: { fontFamily: Fuentes.cuerpo, fontSize: 13, textAlign: 'center', paddingVertical: 40, paddingHorizontal: 24 },
  ...
  chipCat: { borderRadius: 999, borderWidth: 1, paddingHorizontal: 13, paddingVertical: 8 },
});