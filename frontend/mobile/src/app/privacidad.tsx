  ...
  return (
    <View style={{ flex: 1, backgroundColor: temaActivo.canvas }}>
      <Hero paddingTop={insets.top + 14}>
        ...
        <Text style={[s.titulo, { color: temaActivo.blanco }]}>{t('privacidad.titulo')}</Text>
        <Text style={s.subtitulo}>{t('privacidad.actualizado')}</Text>
      </Hero>

      <ScrollView contentContainerStyle={{ padding: Espacio.m, paddingBottom: 40 }}>
        <Tarjeta>
          <Text style={[s.parrafo, { color: temaActivo.tintaSuave }]}>{t('privacidad.intro')}</Text>
          {SECCIONES.map((sec) => (
            <View key={sec} style={{ marginTop: 18 }}>
              {/* 3. Inyectamos colores dinámicos */}
              <Text style={[s.seccionTitulo, { color: temaActivo.tinta }]}>{t(`privacidad.${sec}t`)}</Text>
              <Text style={[s.parrafo, { marginTop: 6, color: temaActivo.tintaSuave }]}>
                {t(`privacidad.${sec}p`)}
              </Text>
            </View>
          ))}
